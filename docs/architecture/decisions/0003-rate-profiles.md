...
Proposed — proposes a `RateProfile` port that expresses the target QPS as a function of elapsed time, three concrete profiles (`SteadyProfile`, `LinearProfile`, `ExponentialProfile`), a small DSL parser (`steady:N`, `linear:A->B@T`, `exp:A->B@T`) exposed via a new `--profile` cmd flag, and an updated Poster loop that recomputes the inter-send interval on every send so the variable rate is honored smoothly. The existing `--qps` flag becomes an alias for `steady:N` so v0.0.1 invocations still work; an operator who passes both `--profile` and `--qps` gets a boot error naming the conflict. The `/metrics` endpoint planned for v0.0.2 moves to v0.0.3 because once a variable rate ships, dashboards become much more valuable, and the metrics shape benefits from being designed after the rate-profile semantics land.
